...
export class Migration1757942570614 implements MigrationInterface {
    name = 'Migration1757942570614'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TYPE "public"."users_role_enum" AS ENUM('driver', 'agent', 'admin')`);
        ...
        await queryRunner.query(`CREATE TYPE "public"."assignment_status" AS ENUM('ASSIGNED', 'COMPLETED', 'CANCELLED')`);
        await queryRunner.query(`CREATE TABLE "assignment" ("driver_id" integer NOT NULL, "load_id" integer NOT NULL, "status" "public"."assignment_status" NOT NULL DEFAULT 'ASSIGNED', "assigned_at" TIMESTAMP NOT NULL DEFAULT now(), "created_at" TIMESTAMP NOT NULL DEFAULT now(), "updated_at" TIMESTAMP DEFAULT now(), CONSTRAINT "PK_9dc6760c01a95a13b4fce22d914" PRIMARY KEY ("driver_id", "load_id"))`);
        await queryRunner.query(`CREATE UNIQUE INDEX "idx_unique_active_load_assignment" ON "assignment" ("load_id", "status") WHERE status = 'ASSIGNED'`);
        await queryRunner.query(`CREATE UNIQUE INDEX "idx_unique_active_assignment" ON "assignment" ("driver_id", "status") WHERE status = 'ASSIGNED'`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`DROP INDEX "public"."idx_unique_active_load_assignment"`);
